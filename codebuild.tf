resource "aws_codebuild_project" "codebuild_project" {
    ...
    artifacts {
      ...
      location = var.s3_bucket == "" ? var.s3_bucket : null
...
resource "aws_codebuild_source_credentials" "codebuild_project" {
    count = var.source_type == "GITHUB_ENTERPRISE" ? 1 : 0
    auth_type = "PERSONAL_ACCESS_TOKEN"
    server_type = "GITHUB_ENTERPRISE"
    token = var.pat_secret_arn
...
resource "github_repository_webhook" "foo" {
  ...
  configuration {
    url = aws.codebuild_webhook.codebuild_project[count.index].payload_url
    content_type = "json"
    insecure_ssl = false
    seccret = aws_codebuild_source_credentials.codebuild_project[count.index].token
  }
}